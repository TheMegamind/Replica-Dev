...
class ReplicaEnergyMonitoringPlug {
    ...
    def initialize() {
        updateDataValue("triggers", JsonOutput.toJson(getReplicaTriggers()))
        updateDataValue("commands", JsonOutput.toJson(getReplicaCommands()))
    }
    ...
    private logInfo(msg) {
        if (settings?.deviceInfoDisable != true) {
            log.info "${msg}"
        }
    }
    ...
    private logTrace(msg) {
        if (settings?.deviceTraceEnable == true) {
            log.trace "${msg}"
        }
    }
